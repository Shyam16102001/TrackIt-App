  ...
  <Tabs default-value="sip" class="w-full">
    ...
    <TabsContent value="sip">
      <div class="flex flex-col md:flex-row">
        <!-- Sliders -->
        <div class="w-full md:w-1/3 p-4 bg-gray-100 dark:bg-black">
          <h3 class="text-xl font-semibold mb-4">SIP Calculator</h3>

          <!-- Instructions -->
          <div class="mb-4 text-sm text-gray-500">
            Adjust the sliders to calculate your SIP future value. You can also type exact values in the boxes.
          </div>
          ...
          <div class="bg-gray-50 dark:bg-black p-4 rounded-md my-4">
        ...
        <div class="w-full md:w-2/3 p-4">
          ...
          <div class="flex space-x-4 mt-4">
            <button @click="viewMode = 'graph'"
              :class="viewMode === 'graph' ? 'bg-gray-800 text-white' : 'bg-gray-300 dark:text-black'"
              class="px-4 py-2 rounded-md">
              Graph View
            </button>
            <button @click="viewMode = 'table'"
              :class="viewMode === 'table' ? 'bg-gray-800 text-white' : 'bg-gray-300 dark:text-black'"
              class="px-4 py-2 rounded-md">
              Table View
            </button>
          </div>
          ...
          <div v-if="viewMode === 'graph'">
            <BarChart :data="chartDataSip" index="name" :categories="['invested', 'predicted']"
...
const totalInterestEarnedSip = computed(() => futureValueSip.value - (monthlyInvestment.value * 12 * timePeriod.value))

// Calculate all-time rate of return
const allTimeRorSip = computed(() => {
  const totalInvestment = monthlyInvestment.value * 12 * timePeriod.value
  return ((futureValueSip.value - totalInvestment) / totalInvestment * 100).toFixed(2)
})
