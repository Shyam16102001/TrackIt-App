<template>
  <CalculatorCardWrapper
    ...
    label="Calculate Compound or Simple Interest"
  >
    <form @submit="onSubmit">
          ...
          <FormControl>
            <RadioGroup v-model="form.contributionType">
              <div class="flex space-x-4">
                ...
                <Label>Withdrawals</Label> <RadioGroupItem value="both" />
                <Label>Both</Label>
              </div>
            </RadioGroup>
          </FormControl>
      ...
      <div v-if="form.contributionType !== 'none'">
            ...
            <FormItem>
              ...
              <FormControl>
                <Select v-model="form.depositFrequency">
                  <SelectTrigger class="w-full">
                    <SelectValue
                      placeholder="Please select Deposit Frequency"
                    />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
        ...
        <div
          v-if="
            form.contributionType === 'withdrawal' ||
            form.contributionType === 'both'
          "
        >
            ...
            <FormItem>
              ...
              <FormControl>
                <Select v-model="form.withdrawalFrequency">
                  <SelectTrigger class="w-full">
                    <SelectValue
                      placeholder="Please select Withdrawal Frequency"
                    />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
      ...
      <div class="mt-6 flex justify-center">
        <Button
          type="submit"
          :disabled="isLoading"
          class="disabled:cursor-not-allowed disabled:bg-gray-300"
        >
      ...
      <div
        v-if="calculated"
        class="mt-6 rounded-lg border border-gray-200 bg-gray-100 p-5 dark:bg-black"
      >
        ...
        <p class="text-lg font-semibold">
          Additional Deposits: {{ formatCurrency(additionalDeposit) }}
        </p>
        <p class="text-lg font-semibold">
          Total Withdrawal Value: {{ formatCurrency(totalWithdrawal) }}
        </p>
      </div>
    </form>
  </CalculatorCardWrapper>
...
const validateForm = () => {
  if (!form.value.rate || !form.value.rateType || !form.value.frequency) {
...
const onSubmit = async (event) => {
  ...
  const years =
    parseFloat(form.value.years) + parseFloat(form.value.months) / 12;
  ...
  let totalPrincipal =
    principal * Math.pow(1 + annualRate / compoundFreq, compoundFreq * years);

  // Contributions
  if (
    form.value.contributionType === "deposit" ||
    form.value.contributionType === "both"
  ) {
    ...
    const depositRate =
      Math.pow(1 + annualRate / compoundFreq, compoundFreq / depositFreq) - 1;

    const depositFutureValue =
      depositAmount *
      ((Math.pow(1 + depositRate, depositPeriods) - 1) / depositRate);
    totalPrincipal += depositFutureValue;
    additionalDeposit.value = depositPeriods * depositAmount;
  }

  if (
    form.value.contributionType === "withdrawal" ||
    form.value.contributionType === "both"
  ) {
    ...
    const withdrawalRate =
      Math.pow(1 + annualRate / compoundFreq, compoundFreq / withdrawalFreq) -
      1;

    const withdrawalFutureValue =
      withdrawalAmount *
      ((Math.pow(1 + withdrawalRate, withdrawalPeriods) - 1) / withdrawalRate);
  ...
  totalInterest.value =
    totalPrincipal -
    principal -
    (additionalDeposit.value - totalWithdrawal.value);
